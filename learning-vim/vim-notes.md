...
- gv可以重复上次visual block的selection，在visual mode下，光标移动的命令都可以用，如$/0/w等, ~可以切换大小写，U可以全部变成大写，u全部变成小写，o命令可以在block首和尾切换
- 在选中block后，执行 :write block.txt可以把block部分写入新文件，执行 ！sort可以对block排序，！后面可以跟其它的外部程序的命令
