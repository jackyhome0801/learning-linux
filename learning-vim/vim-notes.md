...
- Visual Block Mode(按下CTRL+V）下的文本修改，会对所有行都生效，如按I可以插入文本到底一行，然后按ESC，所有行都会自动插入第一行的文本，按下c命令可以删除高亮显示的文本并进入insert mode，在第一行插入新文本之后按ESC可以在所有行都插入新文本，按下C命令删除到行尾而不是仅高亮显示部分
