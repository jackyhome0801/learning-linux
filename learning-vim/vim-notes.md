...
## 多窗口
- 命令  : split 可以把窗口分割 ，CTRL+Ww命令可以切换窗口，CTRL+Wj/Wk可以上下切换窗口，要关闭一个窗口，可以用zz或 :q 命令 ，split后不跟文件名，默认分割出当前文件，也可以带文件名打开其他文件，如当前在one.c ，:split two.c可以让one.c/two.c同时显示出来，也可以跟+commond命令，如 split +/include two.c，可以打开two.c并搜索include
