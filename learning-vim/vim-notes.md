...
- d可以删除高亮显示部分，D可以删除高亮显示的行（即使改行只有部分高亮），y可以copy高亮部分，Y可以copy高亮显示的行，还有c/C命令也可以执行delete之后进入insert mode
...
- Visual Block Mode(按下CTRL+V）下的文本修改，会对所有行都生效，如按I可以插入文本到底一行，然后按ESC，所有行都会自动插入第一行的文本，按下c命令可以删除高亮显示的文本并进入insert mode，在第一行插入新文本之后按ESC可以在所有行都插入新文本，按下C命令删除到行尾而不是仅高亮显示部分，A命令可以在高亮block之后插入文本，如果想在高亮文本的行尾插入文本，可以按CTRL+V$A\<TEXT\>
